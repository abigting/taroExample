...
//app授权
  static appCheckAuth(){
    return new Promise(function (resolve) {
      const state = configStore();
      //如果有授权信息
      if( Auth.checkAuth() && !state.getState().login.appOnLaunch ){
        //直接返回
        resolve(true);
      }else{
        //判断session_key是否过期
  ...
  static checkAuth(){
    const state = configStore();
    //从缓存读取授权信息
    let authorize = state.getState().login.authorize || Taro.getStorageSync('authorize') || {},
        expiryTime = 0,
...
async function getAuthToken(){
  ...
  let response = await Taro.request({
    url : `${state.getState().login.baseURL}api/xxx/xxx` ,
    data : {
      code : res.code
    } ,
    method : 'POST'
